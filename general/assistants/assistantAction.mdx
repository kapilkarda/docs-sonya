...
Ai uses **Zeplo** to send WhatsApp messages. Before using this feature, make sure you have configured your Zeplo credentials in **Integrations → Zeplo** (see [Zeplo Setup Guide](/general/integrations/zeploSetup)).
...
### Here are the steps to sync data between Ai and your CRM (HubSpot, Salesforce and Zapier Hook):

1. After making an outbound or inbound call in Ai, the call data will be automatically updated in your connected CRM platform(s).

2. To manage the CRM sync Action, go to the Ai dashboard click on the agent field and update your agent setting by clicking on the new agent button.

3. In the agent at the bottom right corner you will find Sync Data with CRM.
...
Then, All call data from your outbound or inbound calls in Ai will be automatically synced to your connected HubSpot, Salesforce or Zapier Hook CRM.

### Zapier Hook.

Ai lets you set up actions like booking appointments and sending links through Zapier. Too many actions can confuse the AI, so keep the number of actions small for the best results.

To integrate Ai with Zapier Hook, follow these steps: 

1. Go to the last step of the agent creation process and click on "CRM Data."
...
7. Copy this URL and paste it in the required place in the agent settings, then save it.
...
By following these steps, you can integrate Ai with Zapier and set up automated actions through the Zapier platform.
