...
description: "This document explains how to configure and receive webhooks from the **AI** platform. A webhook lets your AI agent forward collected call information to your own server or any 3rd-party service (CRM, Zapier, Make, your API, Google Sheets, etc.) in real time."
---


## 1. Overview

AI supports **two webhook trigger modes**. You configure them from the **Tools** section of your agent inside the AI Dashboard.
...
**Behind the scenes:**
1. The agent completes the call.
2. AI takes the full transcript + collected context.
...
4. AI sends an HTTP request (method of your choice) to your configured URL with headers, query parameters, and body.
5. Your server responds. The response status + body is logged in the AI delivery history for audit.

---

## 3. Configuring the Webhook in the AI Dashboard

Open **Dashboard → Agents → (select your agent) → Tools → + Add Webhook**. You'll see the following fields:
...
| **Body Parameters** | The fields you want AI to send in the request body. Each parameter has: `Identifier`, `Data Type`, `Required` flag, `Description`, `Value`. |
...
| **Value** | Leave blank. AI will automatically fill this from the call transcript when the call ends. |
...
**Webhook URL example:** `https://your-domain.com/api/call-report`
**Method:** `POST`
...
When the call ends, AI sends a request like this to the URL you configured:

**Request**
```
POST /api/call-report HTTP/1.1
...
```json
{
  ...
  "recording_url": "https://cdn.xyz.com/recordings/cs_abc123.mp3"
}
```
...
Your server should respond with **HTTP 200 or 201** and a JSON body. Non-2xx responses are logged as failures in AI's delivery history.
...
AI automatically serializes the body based on the `Content-Type` header you set:
...
2. **Use the webhook.site URL** as the Webhook URL in the AI Dashboard while testing:
   ```
   https://webhook.site/<your-unique-id>
   ```
3. **Place a test call** through the AI dashboard or playground.
...
app.post("/api/call-report", (req, res) => {
...
Yes. Add multiple webhook tools under the same agent. Each one has its own URL, method, and parameter set.
...
Not natively — every call for that agent fires the webhook. Filter on your side using `call_outcome` or `sentiment_analysis`.

**Q. Where can I see webhook delivery history?**
Contact your AI account manager for access to the delivery log for your workspace.
...
For issues, error codes, or custom integrations, reach out to your AI support contact with:
- Agent name
- Approximate call timestamp (with timezone)
- Your server's response code + body

We will match it against AI's delivery log for debugging.
