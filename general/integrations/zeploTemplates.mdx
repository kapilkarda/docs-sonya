...
description: "Learn how to configure Zeplo WhatsApp templates, parameter hints, and send messages from Ai agents."
---

## Zeplo Template Configuration in Ai

Once your Zeplo credentials are configured (see [Zeplo Setup Guide](/general/integrations/zeploSetup)), you can use WhatsApp templates in your Ai agents to send automated messages.

---

### How Templates Work

Zeplo templates are WhatsApp Business API message templates that contain placeholders like `{{1}}`, `{{2}}`, etc. When Ai sends a message, these placeholders are filled with real values extracted from the call conversation.
...
### Create an Agent with Zeplo

To use Zeplo WhatsApp messaging in your AI agent, you need to add a **Send Message** action and configure the Zeplo card. Below are all the steps and required fields.

#### Step 1: Create or Edit an Agent

1. Navigate to **Agents** from the left sidebar menu.
2. Click **Create Agent** to create a new one, or click an existing agent to edit it.
3. Fill in the basic agent details (name, prompt, voice, etc.).
...
After selecting a template, Ai automatically parses all placeholders and displays **Parameter Hints** input fields. These hints tell the AI what each placeholder represents so it can extract the correct value from the conversation.
...
Authentication templates have special handling. Meta requires the **same OTP value** to appear in both the body and button parameters. Ai handles this automatically:
...
3. Ai will automatically replicate the OTP to all body and button placeholders at send time.
...
After sending messages through Ai campaigns, you can track the Zeplo delivery status in multiple places:
...
Ai uses the following backend endpoints to communicate with Zeplo:
