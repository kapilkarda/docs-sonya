...
description: "Learn how to configure Zeplo WhatsApp templates, parameter hints, and send messages from Bolify AI assistants."
---

## Zeplo Template Configuration in Bolify AI

Once your Zeplo credentials are configured (see [Zeplo Setup Guide](/general/integrations/zeploSetup)), you can use WhatsApp templates in your Bolify AI assistants to send automated messages.

---

### How Templates Work

Zeplo templates are WhatsApp Business API message templates that contain placeholders like `{{1}}`, `{{2}}`, etc. When Bolify AI sends a message, these placeholders are filled with real values extracted from the call conversation.
...
After selecting a template, Bolify AI automatically parses all placeholders and displays **Parameter Hints** input fields. These hints tell the AI what each placeholder represents so it can extract the correct value from the conversation.
...
Authentication templates have special handling. Meta requires the **same OTP value** to appear in both the body and button parameters. Bolify AI handles this automatically:
...
3. Bolify AI will automatically replicate the OTP to all body and button placeholders at send time.
...
After sending messages through Bolify AI campaigns, you can track the Zeplo delivery status in multiple places:
...
Bolify AI uses the following backend endpoints to communicate with Zeplo:
