...
## Plivo SIP Integration Guide — Bolify SIP Dialer (Inbound + Outbound)

This guide walks a customer through connecting a **Plivo SIP trunk** to Bolify so their
...
- **Part A — Plivo side:** buy a number, create an Outbound Trunk, generate SIP credentials, and get the
  values Bolify needs.
- **Part B — Bolify side:** add the SIP Dialer in the Bolify dashboard, then finish the inbound
  loop-back on Plivo.

> **You need one thing from each side.** Plivo gives you a *SIP termination domain + username + password +
> phone number*. Bolify gives you back a *SIP URI* that Plivo must forward inbound calls to. That
...
```
   ...
   Bolify → Plivo termination URI ─────────────► sends INVITE to
   ...
   your Bolify SIP URI  ────────────────► Inbound Trunk (matches the number)
                                                 ──► Dispatch Rule ──► AI agent answers
```

- **Outbound = Bolify → Plivo.** From QCall's point of view, Plivo is where the call *terminates*
  into the phone network. That is why Bolify calls it the **SIP Termination URI**.
...
## Credential cheat-sheet (Plivo → Bolify field mapping)

Fill this in as you go through Part A. You'll paste the right-hand column into the Bolify dashboard
in Part B.

| What you get from Plivo                     | Example value                     | Bolify dashboard field    |
| ------------------------------------------- | --------------------------------- | ------------------------ |
...
| What Bolify gives you           | Example value                       | Where it goes in Plivo                  |
| ------------------------------ | ----------------------------------- | --------------------------------------- |
| Your **SIP URI**       | `myproj-x1y2.sip.livekit.cloud`     | Inbound Trunk **Primary URI**           |

> The exact SIP URI is shown on your dialer's **SIP configuration** in the Bolify dashboard
...
The Outbound Trunk is what Bolify sends outbound calls **into** — it is the source of your
**SIP Termination URI**.
...
3. For **Trunk Authentication**, choose **Credentials List** (Bolify authenticates with
...
> Bolify authenticates with **username/password (digest auth)**, so the Credentials List is required.
...
in Step 2. (You'll set its **Primary URI** later in Part C, once Bolify gives you the SIP URI.)
...
# Part B — Bolify dashboard: add the SIP Dialer

This is what a customer actually clicks in the Bolify app.

1. Log in to the **Bolify dashboard**.
...
**What Bolify does behind the scenes** (no action needed from you):
...
**What Bolify gives you back:** on success, the dialer now exists and Bolify exposes your
**SIP URI** (shown in the dialer's SIP config / setup instructions, format
`178.212.35.170:5060`). **Copy it — you need it for Part C.**

> If you don't see the SIP URI in the UI, it's a fixed value for your Bolify workspace — ask QCall
...
1. Return to the **Plivo console** → **SIP Trunking → Inbound Trunks** → open the inbound trunk from
   Step 5.
2. Set the **Primary URI** to your Bolify **SIP URI** from Part B:
...
<Frame>
    <img src="/images/plivo/08-primary-uri.jpeg" alt="Plivo console: Inbound Trunk Primary URI set to the Bolify SIP URI with ;transport=tcp" style={{ borderRadius: '0.5rem' }} />
</Frame>

Now the loop is closed:

- **Outbound:** Bolify → `21784177241578.zt.plivo.com` (your Plivo trunk) → PSTN.
...
| "This phone number is already being used"           | The DID is already registered to another Bolify dialer/workspace. Delete the old dialer or use a new DID. |
...
| Password stopped working                            | Plivo passwords are write-once. If rotated/lost, create a new credential and update the Bolify dialer.  |
...
- [Bolify — Vobiz SIP Integration](https://docs.qcall.ai/general/dialer/vobizSipIntegration) (sister guide)
