...
## Plivo SIP Integration Guide — AI SIP Dialer (Inbound + Outbound)

This guide walks a customer through connecting a **Plivo SIP trunk** to AI so their
...
- **Part A — Plivo side:** buy a number, create an Outbound Trunk, generate SIP credentials, and get the
  values AI needs.
- **Part B — AI side:** add the SIP Dialer in the AI dashboard, then finish the inbound
  loop-back on Plivo.

> **You need one thing from each side.** Plivo gives you a *SIP termination domain + username + password +
> phone number*. AI gives you back a *SIP URI* that Plivo must forward inbound calls to. That
...
```
   ...
   AI → Plivo termination URI ─────────────► sends INVITE to
   ...
   your AI SIP URI  ────────────────► Inbound Trunk (matches the number)
                                                 ──► Dispatch Rule ──► AI agent answers
```

- **Outbound = AI → Plivo.** From AI's point of view, Plivo is where the call *terminates*
  into the phone network. That is why AI calls it the **SIP Termination URI**.
- **Inbound = Plivo → AI.** Plivo must be told to *originate* the incoming call toward AI's
  SIP address (set as the Plivo **Inbound Trunk Primary URI**).

---

## Credential cheat-sheet (Plivo → AI field mapping)

Fill this in as you go through Part A. You'll paste the right-hand column into the AI dashboard
in Part B.

| What you get from Plivo                     | Example value                     | AI dashboard field    |
| ------------------------------------------- | --------------------------------- | ------------------------ |
| Outbound Trunk **termination SIP domain**   | `21784177241578.zt.plivo.com`     | **SIP Termination URI**  |
| Outbound Trunk **credential username**      | `prod`                       | **SIP Username**         |
...
Then, after saving in AI, you receive **one** value to bring *back* to Plivo:

| What AI gives you           | Example value                       | Where it goes in Plivo                  |
| ------------------------------ | ----------------------------------- | --------------------------------------- |
| Your **SIP URI**       | `myproj-x1y2.sip.livekit.cloud`     | Inbound Trunk **Primary URI**           |

> The exact SIP URI is shown on your dialer's **SIP configuration** in the AI dashboard
...
2. After login, open the dashboard. Plivo's SIP trunking product is called **Zentrunk** — you'll find
   ...
   (these authenticate Plivo API calls; you do **not** paste them into AI).
...
4. Record it in **E.164 format with the `+` and country code** — e.g. `+12025550147`.
   → This becomes the **Phone Number** field in AI.
...
The Outbound Trunk is what AI sends outbound calls **into** — it is the source of your
**SIP Termination URI**.

1. Go to **SIP Trunking → Outbound Trunks** and click **Create New Outbound Trunk**.
2. Under **Trunk Details**, enter a name (e.g. `ai-outbound`).
3. For **Trunk Authentication**, choose **Credentials List** (AI authenticates with
...
5. On save, Plivo provisions a unique **termination SIP domain** in the form
   ...
   → This becomes the **SIP Termination URI** in AI.
...
2. Enter an **Auth Group** name (e.g. `ai-creds`), a **username**, and a **password**.
   - Username: **5–20 alphanumeric** characters (e.g. `prod`).
...
in Step 2. (You'll set its **Primary URI** later in Part C, once AI gives you the SIP URI.)

1. Go to **SIP Trunking → Inbound Trunks** and click **Create New Inbound Trunk**.
2. Enter a name (e.g. `ai-inbound`).
...
# Part B — AI dashboard: add the SIP Dialer

This is what a customer actually clicks in the AI app.
...
4. Fill in the form using your cheat-sheet:
   ...
   | **SIP Username**     | The Plivo credential username, e.g. `prod`            |
   | **SIP Password**     | The Plivo credential password (paste the one you copied)   |

5. Click **Create / Save**.

**What AI does behind the scenes** (no action needed from you):
...
**What AI gives you back:** on success, the dialer now exists and AI exposes your
**SIP URI** (shown in the dialer's SIP config / setup instructions, format
`178.212.35.170:5060`). **Copy it — you need it for Part C.**

> If you don't see the SIP URI in the UI, it's a fixed value for your AI workspace — ask AI
...
1. Return to the **Plivo console** → **SIP Trunking → Inbound Trunks** → open the inbound trunk from
   Step 5.
2. Set the **Primary URI** to your AI **SIP URI** from Part B:
...
<Frame>
    <img src="/images/plivo/08-primary-uri.jpeg" alt="Plivo console: Inbound Trunk Primary URI set to the AI SIP URI with ;transport=tcp" style={{ borderRadius: '0.5rem' }} />
</Frame>

Now the loop is closed:

- **Outbound:** AI → `21784177241578.zt.plivo.com` (your Plivo trunk) → PSTN.
- **Inbound:** Caller → your DID → Plivo → `178.212.35.170:5060` (AI) → AI agent.
...
**Outbound test**
1. In AI, assign the new SIP dialer to a campaign or a Playground test call.
...
2. It should connect to your AI agent (assign an inbound assistant to this dialer/number in AI
   first).
...
| Outbound rejected (number/format)                   | DID must be E.164 with `+` in AI. Confirm the DID is Voice-capable on Plivo.                        |
...
| "This phone number is already being used"           | The DID is already registered to another AI dialer/workspace. Delete the old dialer or use a new DID. |
...
| Password stopped working                            | Plivo passwords are write-once. If rotated/lost, create a new credential and update the AI dialer.  |
