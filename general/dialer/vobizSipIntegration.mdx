...
## Vobiz SIP Integration Guide — Bolify SIP Dialer (Inbound + Outbound)

This guide walks a customer through connecting a **Vobiz SIP trunk** to Bolify so their
...
> number*. Bolify gives you back a *SIP URI* that Vobiz must forward inbound calls to. That
...
```
   ...
   Bolify → Vobiz SIP domain (termination) ────► sends INVITE to
...
- **Outbound = Bolify → Vobiz.** From QCall's point of view, Vobiz is where the call *terminates*
...
## Credential cheat-sheet (Vobiz → Bolify field mapping)

Fill this in as you go through Part A. You'll paste the right-hand column into the Bolify dashboard
in Part B.

| What you get from Vobiz            | Example value                     | Bolify dashboard field    |
| ---------------------------------- | --------------------------------- | ------------------------ |
...
| What Bolify gives you           | Example value                       | Where it goes in Vobiz            |
| ------------------------------ | ----------------------------------- | --------------------------------- |
...
> Vobiz also supports IP-based auth. Bolify authenticates with **username/password**, so credential
...
2. Leave the inbound destination for now — you'll point it at Bolify in **Part C**, after QCall
...
2. Set the inbound **destination / origination URI** to your Bolify **SIP URI** from Part B.
...
<Frame>
    <img src="/images/vobiz/06-inbound-origination.png" alt="Vobiz console: inbound destination / origination URI set to the Bolify SIP URI" style={{ borderRadius: '0.5rem' }} />
</Frame>

Now the loop is closed:

- **Outbound:** Bolify → `abc123.sip.vobiz.ai` (your Vobiz trunk) → PSTN.
...
| Password stopped working                            | Vobiz passwords are write-once. If rotated/lost, create a new Vobiz credential and update the Bolify dialer. |
