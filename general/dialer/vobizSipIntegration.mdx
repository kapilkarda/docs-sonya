...
## Vobiz SIP Integration Guide — AI SIP Dialer (Inbound + Outbound)

This guide walks a customer through connecting a **Vobiz SIP trunk** to AI so their
...
> number*. AI gives you back a *SIP URI* that Vobiz must forward inbound calls to. That
...
```
   ...
   AI → Vobiz SIP domain (termination) ────► sends INVITE to
...
- **Outbound = AI → Vobiz.** From AI's point of view, Vobiz is where the call *terminates*
  into the phone network. That is why AI calls it the **SIP Termination URI**.
- **Inbound = Vobiz → AI.** Vobiz must be told to *originate* the incoming call toward AI's
  SIP address.

---

## Credential cheat-sheet (Vobiz → AI field mapping)

Fill this in as you go through Part A. You'll paste the right-hand column into the AI dashboard
in Part B.

| What you get from Vobiz            | Example value                     | AI dashboard field    |
| ---------------------------------- | --------------------------------- | ------------------------ |
| Trunk **SIP domain**               | `abc123.sip.vobiz.ai`             | **SIP Termination URI**  |
| Trunk **credential username**      | `prod`                      | **SIP Username**         |
...
Then, after saving in AI, you receive **one** value to bring *back* to Vobiz:

| What AI gives you           | Example value                       | Where it goes in Vobiz            |
| ------------------------------ | ----------------------------------- | --------------------------------- |
...
2. After login, open the dashboard. Note your account-level **Auth ID** and **Auth Token** — these
   authenticate any Vobiz API calls. (You don't paste these into AI; they're only for managing
   Vobiz itself.)
...
2. Give it a name (e.g. `ai-trunk`) and set the trunk type to support both directions
...
2. Set a **username** (e.g. `prod`) and a **strong password** (12+ chars, mixed case, digits,
   symbols).
...
> Vobiz also supports IP-based auth. AI authenticates with **username/password**, so credential
...
2. Leave the inbound destination for now — you'll point it at AI in **Part C**, after AI
...
4. Fill in the form using your cheat-sheet:
   ...
   | **SIP Username**     | The Vobiz credential username, e.g. `prod`          |
...
2. Set the inbound **destination / origination URI** to your AI **SIP URI** from Part B.
...
<Frame>
    <img src="/images/vobiz/06-inbound-origination.png" alt="Vobiz console: inbound destination / origination URI set to the AI SIP URI" style={{ borderRadius: '0.5rem' }} />
</Frame>

Now the loop is closed:

- **Outbound:** AI → `abc123.sip.vobiz.ai` (your Vobiz trunk) → PSTN.
- **Inbound:** Caller → your DID → Vobiz → `178.212.35.170:5060` (AI) → AI agent.
...
| Outbound rejected (number/format)                   | DID must be E.164 with `+` in AI. Confirm the DID is enabled for outbound on Vobiz.                  |
...
| Password stopped working                            | Vobiz passwords are write-once. If rotated/lost, create a new Vobiz credential and update the AI dialer. |
